    ...
    <main className="min-h-screen" style={{ background: '#EFF7FC' }}>
      ...
      <section id="why-join" className="py-20 px-6 scroll-mt-24">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[#1D7ED0] font-semibold text-sm uppercase tracking-widest mb-3">Why join</p>
            <h2 className="text-3xl font-bold text-[#0B3558]">A better way to clean for a living</h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {BENEFITS.map(b => (
              <div key={b.title} className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-2xl p-6 shadow-sm">
                <div className="text-3xl mb-3">{b.icon}</div>
                <h3 className="font-semibold text-[#0B3558] mb-2">{b.title}</h3>
                <p className="text-[#60798F] text-sm leading-relaxed">{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6" style={{ background: 'linear-gradient(180deg, #EFF7FC 0%, #DCECF8 100%)' }}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl font-bold text-[#0B3558]">How to get started</h2>
          </div>
      ...
      <section className="py-20 px-6 gradient-cta">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link href="/careers/apply" className="inline-flex items-center gap-2 bg-white text-[#0B3558] font-bold px-10 py-4 rounded-full text-lg hover:bg-white/90 transition-all hover:-translate-y-1">
            Apply now →
          </Link>
        </div>
      </section>
