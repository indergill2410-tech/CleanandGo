  ...
  return (
    <main className="min-h-screen" style={{ background: '#F5F0EB' }}>
      ...
      <section className="gradient-hero pt-36 pb-24 px-6 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-10 w-80 h-80 rounded-full opacity-15 blur-3xl" style={{ background: '#4A7FA5' }} />
          ...
          {STEPS.map((step, i) => (
            <div key={step.number} className="flex flex-col md:flex-row gap-6 items-start">
              <div className="flex-shrink-0 flex md:flex-col items-center gap-4">
                ...
                {i < STEPS.length - 1 && (
                  <div className="hidden md:block w-px h-12 mx-auto" style={{ background: 'linear-gradient(180deg, #4A7FA5, transparent)' }} />
                )}
              </div>
              <div className="flex-1 pt-1">
                <div className="text-[#7BA7C7] font-bold text-xs tracking-widest mb-2">STEP {step.number}</div>
                <h2 className="text-2xl font-bold text-[#1C2B3A] mb-2">{step.title}</h2>
                <p className="text-[#4A7FA5] font-medium mb-3">{step.desc}</p>
                <p className="text-[#7A8A96] leading-relaxed">{step.detail}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-20 px-6" style={{ background: 'linear-gradient(180deg, #F5F0EB 0%, #EBF0F5 100%)' }}>
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[#4A7FA5] font-semibold text-sm uppercase tracking-widest mb-3">Good to know</p>
            <h2 className="text-3xl font-bold text-[#1C2B3A]">Frequently asked questions</h2>
          </div>
