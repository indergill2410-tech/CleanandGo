        ...
        {(['client', 'staff'] as const).map(t => (
          <button key={t} type="button" onClick={() => switchTab(t)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-colors ${
              tab === t ? 'bg-white text-[#0B3558]' : 'text-white/60 hover:text-white'
            }`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {tab === 'client' && mode === 'signup' && (
          <label className="flex items-start gap-2.5 text-white/70 text-sm cursor-pointer">
            <input type="checkbox" checked={optIn} onChange={e => setOptIn(e.target.checked)} className="w-4 h-4 mt-0.5 accent-[#1D7ED0]" />
        ...
        <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl bg-white text-[#0B3558] font-bold hover:bg-white/90 transition disabled:opacity-50">
