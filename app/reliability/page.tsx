        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-8">
          {PROMISES.map(p => (
            <div key={p.title} className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-3xl p-8 shadow-lg">
              ...
              <h2 className="text-lg font-bold text-[#1C2B3A] mb-2">{p.title}</h2>
              <p className="text-[#7A8A96] text-sm leading-relaxed">{p.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-6" style={{ background: 'linear-gradient(180deg, #F5F0EB 0%, #EBF0F5 100%)' }}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[#4A7FA5] font-semibold text-sm uppercase tracking-widest mb-3">Backup coverage</p>
            <h2 className="text-3xl font-bold text-[#1C2B3A]">How we keep your clean on track</h2>
          </div>
