...
export default function BookingPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #0B3558 0%, #0B3558 50%, #1D7ED0 100%)' }}>
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
        <div className="absolute top-20 right-20 w-80 h-80 rounded-full blur-3xl opacity-20" style={{ background: '#7DD3FC' }} />
        <div className="absolute bottom-20 left-20 w-96 h-96 rounded-full blur-3xl opacity-10" style={{ background: '#1D7ED0' }} />
      </div>
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-4 py-8 sm:py-12">
        ...
        <div className="w-full max-w-xl mb-6 sm:mb-8">
            ...
            {STEPS.map((s, i) => (
              <div key={i} className="flex min-w-0 flex-col items-center gap-1">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                  i < step ? 'bg-white text-[#0B3558]' : i === step ? 'bg-white text-[#0B3558] ring-4 ring-white/30' : 'bg-white/20 text-white/50'
                }`}>{i < step ? '✓' : i + 1}</div>
          ...
          <div className="h-1 bg-white/20 rounded-full">
            <div className="h-1 rounded-full transition-all duration-500" style={{ width: `${(step / (STEPS.length - 1)) * 100}%`, background: 'linear-gradient(90deg, #7DD3FC, white)' }} />
          </div>
        </div>
        ...
        <div className="glass-strong w-full max-w-xl rounded-2xl p-5 shadow-2xl sm:rounded-3xl sm:p-8">
                  ...
                  {TIMES.map(t => (
                    <button key={t} onClick={() => setForm(f => ({ ...f, time: t }))}
                      className={`py-2.5 rounded-xl text-sm font-medium transition-all ${
                        form.time === t ? 'bg-white text-[#0B3558] font-bold' : 'bg-white/10 text-white/70 border border-white/20 hover:bg-white/20'
                      }`}>{t}</button>
                  ))}
                </div>
              </div>
          ...
          <div className="flex flex-col gap-3 mt-6 sm:flex-row">
            ...
            {step < STEPS.length - 1 && (
              <button disabled={!canNext} onClick={() => setStep(s => s + 1)} className="flex-1 py-3 rounded-xl font-semibold transition-all disabled:opacity-30 bg-white text-[#0B3558] hover:bg-white/90">Continue →</button>
            )}
          </div>
        </div>
      </div>
