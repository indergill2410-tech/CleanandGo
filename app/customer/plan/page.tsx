...
export default function PlanRequestPage() {
  ...
  return (
    <div className="min-h-screen py-12 px-4" style={{ background: 'linear-gradient(135deg, #1C2B3A 0%, #2C4A6E 50%, #4A7FA5 100%)' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['home', 'office'] as PropertyType[]).map(t => (
                <button key={t} type="button" onClick={() => setPropertyType(t)}
                  className={`py-3 rounded-xl text-sm font-semibold capitalize transition ${propertyType === t ? 'bg-white text-[#2C4A6E]' : 'bg-white/10 text-white/70'}`}>
                  {t === 'home' ? '🏠 Home' : '🏢 Office'}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {(['weekly', 'fortnightly', 'monthly'] as Frequency[]).map(f => (
                <button key={f} type="button" onClick={() => setFrequency(f)}
                  className={`py-3 rounded-xl text-xs font-semibold capitalize transition ${frequency === f ? 'bg-white text-[#2C4A6E]' : 'bg-white/10 text-white/70'}`}>
                  {f}
                </button>
              ))}
            </div>
