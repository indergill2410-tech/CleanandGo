    ...
    <main className="min-h-screen" style={{ background: '#F7F3EE' }}>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-[#172434]">Why Australian businesses switch to us</h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {REASONS.map(r => (
              <div key={r.title} className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-2xl p-6 shadow-sm">
                <div className="text-3xl mb-3">{r.icon}</div>
                <h3 className="font-semibold text-[#172434] mb-2">{r.title}</h3>
                <p className="text-[#5F6E78] text-sm leading-relaxed">{r.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6" style={{ background: 'linear-gradient(180deg, #F7F3EE 0%, #EFE7DC 100%)' }}>
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-10">
            <p className="text-[#2F7D6B] font-semibold text-sm uppercase tracking-widest mb-3">Scope</p>
            <h2 className="text-3xl font-bold text-[#172434]">What a standard office clean covers</h2>
          </div>
          <div className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-3xl p-8 shadow-lg grid sm:grid-cols-2 gap-3">
            {INCLUDED.map(item => (
              <div key={item} className="flex items-start gap-3 text-sm text-[#172434]">
                <span className="text-[#2F7D6B] mt-0.5">✓</span>{item}
              </div>
            ))}
          </div>
          <p className="text-center text-[#5F6E78] text-sm mt-6">Bespoke scope and frequency? We tailor every commercial quote to your site.</p>
        </div>
      </section>
      ...
      <section id="airbnb" className="py-20 px-6">
        <div className="max-w-5xl mx-auto grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="text-[#2F7D6B] font-semibold text-sm uppercase tracking-widest mb-3">For Airbnb hosts</p>
            <h2 className="text-3xl md:text-4xl font-bold text-[#172434] mb-5 leading-tight">
              Guest leaves at 10. Next guest arrives at 3. We reset everything between.
            </h2>
            <p className="text-[#5F6E78] leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-8">
              <Link href="/customer/plan" className="inline-flex items-center justify-center gap-2 bg-[#172434] text-white font-bold px-8 py-4 rounded-full hover:bg-[#172434] transition-all hover:-translate-y-0.5">
                Request Host Plan →
              </Link>
              <Link href="/customer/book" className="inline-flex items-center justify-center gap-2 border-2 border-[#172434] text-[#172434] font-bold px-8 py-4 rounded-full hover:bg-[#172434] hover:text-white transition-all">
                Book One Turnover
              </Link>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-3xl p-8 shadow-lg">
            ...
            <div className="mt-6 rounded-2xl bg-[#F7F3EE] p-5">
              <div className="text-[#172434] font-bold">Flexible service options</div>
              <p className="text-[#5F6E78] text-sm mt-2 leading-relaxed">
