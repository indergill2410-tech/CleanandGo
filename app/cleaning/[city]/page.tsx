      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-[#1C2B3A]">Cleaning services in {c.name}</h2>
            <p className="text-[#7A8A96] mt-3">Whatever your home or office needs, we’ve got a {c.name} cleaner for it.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {SERVICES.map((s) => (
              <div key={s.slug} className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-2xl p-6 shadow-sm">
                <div className="text-3xl mb-3">{s.icon}</div>
                <h3 className="font-bold text-[#1C2B3A] mb-2">{s.title}</h3>
                <p className="text-[#7A8A96] text-sm leading-relaxed">{s.tagline}</p>
              </div>
            ))}
          </div>
        </div>
