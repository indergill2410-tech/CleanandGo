...
export default function Navbar() {
  ...
  const linkColor = solid ? 'text-[#60798F] hover:text-[#0B3558]' : 'text-white/80 hover:text-white'
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-3">
        <Link href="/" onClick={() => setMenuOpen(false)} className="flex min-w-0 items-center gap-2">
          ...
          <span className={`font-bold text-lg tracking-tight ${solid ? 'text-[#0B3558]' : 'text-white'}`}>cleanngo</span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <Link
            ...
            className={`hidden sm:inline-flex items-center rounded-full text-sm font-bold px-5 py-2.5 sm:px-6 sm:py-3 border-2 transition-all ${
              solid
                ? 'border-[#0B3558] text-[#0B3558] hover:bg-[#0B3558] hover:text-white'
                : 'border-white/60 text-white hover:bg-white hover:text-[#0B3558]'
              }`}
          >Log in</Link>
          ...
          >
            <span className={`block w-5 h-0.5 rounded-full transition-all ${solid ? 'bg-[#0B3558]' : 'bg-white'} ${menuOpen ? 'translate-y-2 rotate-45' : ''}`} />
            <span className={`block w-5 h-0.5 rounded-full transition-all ${solid ? 'bg-[#0B3558]' : 'bg-white'} ${menuOpen ? 'opacity-0' : ''}`} />
            <span className={`block w-5 h-0.5 rounded-full transition-all ${solid ? 'bg-[#0B3558]' : 'bg-white'} ${menuOpen ? '-translate-y-2 -rotate-45' : ''}`} />
          </button>
        </div>
      </div>
          ...
          <div className="max-w-6xl mx-auto px-4 py-4 sm:px-6 flex flex-col gap-1">
            {LINKS.map(item => (
              <Link key={item.label} href={item.href} onClick={() => setMenuOpen(false)}
                className="py-3 text-[#0B3558] font-medium border-b border-black/5 last:border-0">
                {item.label}
              </Link>
            ))}
            <Link href="/account" onClick={() => setMenuOpen(false)}
              className="mt-3 text-center rounded-full font-bold px-6 py-3 border-2 border-[#0B3558] text-[#0B3558] hover:bg-[#0B3558] hover:text-white transition-all">
              Log in / Sign up
            </Link>
